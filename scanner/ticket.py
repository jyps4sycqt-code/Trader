...
def write_markdown(
    ...
    lines.append("")
    lines.append("1. Trade > Stocks/ETFs.")
    lines.append("2. For each row above:")
    lines.append(f"   - Action: Buy   - Symbol: as listed   - Quantity: shares")
    lines.append(f"   - Order Type: **{order_type}**, Time-in-Force: Day")
    lines.append(f"   - If MOO is unavailable, use **{fallback_order_type}** at the listed Limit Fallback price.")
    lines.append("3. Enter sells for the close-rows the same way at Market / Day.")
    lines.append("")
    lines.append("## Basket import (Active Trader Pro)")
    lines.append("")
    lines.append("Open Active Trader Pro > Trade > Baskets > Import. Select the `trade_ticket.csv`")
    lines.append("in this folder and confirm orders.")
    p.write_text("\n".join(lines))
    log.info("wrote %s", p)
    return p
